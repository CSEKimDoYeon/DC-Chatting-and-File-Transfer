...
public:
	...
	typedef struct _ETHERNET_HEADER {
		...
		ETHERNET_ADDR	enet_dstaddr;		// 이더넷 레이어의 상대방 주소 // 6byte
		ETHERNET_ADDR	enet_srcaddr;		// 이더넷 레이어의 본인의 주소 // 6byte
		unsigned short	enet_type;			// 프레임 데이터 타입 // 2byte
		unsigned char	enet_data[ ETHER_MAX_DATA_SIZE ] ; // frame data // 46~1500 byte
